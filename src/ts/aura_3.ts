...
(async () => {
    ...
    registerAuraService({
        writeAuraShare: (peer_id: string, username: string, share: string, next_holding: any) => {
        ...
        retrieveShare: (username, hop_packet) => {
            console.log('retrieving shares')
            console.log(shares)
            console.log(hop_packet)
            console.log(shares[username].share)
            return {
                username: username,
                shares: [...hop_packet.shares, shares[username].share],
                next_peers: shares[username].next_holding
            }
        }
    })

    await register('12D3KooWQ7P4MB1MStsUDBFn6XNv8SpteBC4V5YvW1qYr4PJ1ryw')
