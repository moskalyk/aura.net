...
function App() {
  ...
  const signInClick = async () => {
    const res1 = await signIn('12D3KooWQ7P4MB1MStsUDBFn6XNv8SpteBC4V5YvW1qYr4PJ1ryw', username, password, {ttl: 20000})
    console.log(res1)
    ...
    const recovered = await sss.combine([u8arr1,u8arr2, u8arr3])
  ...
  useEffect(() => {
    setTimeout(async () => {
      await Fluence.connect(kras[9])
    }, 0)
  }, [])

  return (
    <div className="App">
      <input placeholder='username' onChange={(evt:any) => setUsername(evt.target.value)}></input>
      <input placeholder='password' onChange={(evt:any) => setPassword(evt.target.value)}></input>
